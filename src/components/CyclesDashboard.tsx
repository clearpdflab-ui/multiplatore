...
import {
  AlertTriangle, Ban, Check, Plus, Trash2, Wallet, X,
} from 'lucide-react';
...
export const CyclesDashboard: React.FC = () => {
  const {
    ...
    createCycle, closeCycle, deleteCycle, topUpCycle,
  ...
  const conflicts = useMemo(
    () => findSharedLegs(active.map((c) => ({ cycleId: c.id, legKeys: pendingLegKeys(tickets, c.id) }))),
    [active, tickets],
  );
  ...
  const preview = useMemo(() => {
    if (!parsed.legs || !book) return null;
    try {
  ...
  const termination = useMemo((): {
    ...
    if (!selected || !book) return { res: null, ceiling: null, error: null };
    const over = Number(termOver);
    const n = Number(termN);
    if (!Number.isFinite(over) || over <= 1) return { res: null, ceiling: null, error: 'Over non valido' };
    const rest = selected.motherEvents.map((m) => m.odds);
    const full = [{ odds: over, market: 'OVER' as const }, ...rest.map((q) => ({ odds: q, market: 'UNDER' as const }))];
    ...
    if (short) candidates.push({ label: `terminazione N=${n}`, legs: short, book });
    candidates.push({ label: 'lock @2.75', legs: [{ odds: 2.75, market: 'OVER' as const }], book });
    const bleedNum = Number(bleed) || 0;
    const res = resolveWithFallback({ candidates, spent, bleed: bleedNum, targetBase: target, minStake: book.minStake });
  ...
  if (loading) return <div className="text-xs font-mono text-[#64748B]">Caricamento cicli…</div>;

  return (
    <div className="space-y-4">
      {usingFallback && (
        <div className="text-[11px] font-mono text-amber-400 border border-amber-400/30 bg-amber-400/5 rounded-xs px-3 py-2">
          Solo locale (nessuna sessione Supabase): i cicli restano su questo browser. Fai login per la sync cloud.
        </div>
      )}
      {(msg || err || error) && (
        <div className={`text-[11px] font-mono rounded-xs px-3 py-2 border ${err || error ? 'text-red-400 border-red-400/30 bg-red-400/5' : 'text-emerald-400 border-emerald-400/30 bg-emerald-400/5'}`}>
          {err ?? error ?? msg}
        </div>
      )}
      ...
      <section className={cardCls}>
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input value={bankroll} onChange={(e) => setBankroll(e.target.value)} className={`${inputCls} w-28`} inputMode="decimal" />
          </div>
          <div>
            <label className={labelCls}>Worst cost ciclo €</label>
            <input value={worstCost} onChange={(e) => setWorstCost(e.target.value)} className={`${inputCls} w-28`} inputMode="decimal" />
          </div>
          ...
          <div className="mt-2 text-[11px] font-mono text-red-400 flex flex-col gap-1">
            {conflicts.map((c) => (
              <span key={c.key} className="flex items-center gap-1.5">
                ...
                Shared leg “{c.key}” in {c.cycleIds.length} cicli ({c.cycleIds.join(', ')}) — correlazione vietata
              </span>
            ))}
          </div>
        )}
      ...
      <section className={cardCls}>
        <h2 className="text-sm font-semibold text-white mb-2">Cicli ({active.length} attivi / {cycles.length})</h2>
        <div className="flex flex-wrap gap-2 mb-3">
          {cycles.length === 0 && <span className="text-[11px] font-mono text-[#64748B]">Nessun ciclo. Creane uno.</span>}
          {cycles.map((c) => (
            <button
              key={c.id}
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ciclo 1" className={`${inputCls} w-32`} />
          </div>
          <div>
            <label className={labelCls}>S0 €</label>
            <input value={newS0} onChange={(e) => setNewS0(e.target.value)} className={`${inputCls} w-20`} inputMode="decimal" />
          </div>
          <div>
            <label className={labelCls}>Under madre</label>
            <input value={newUnder} onChange={(e) => setNewUnder(e.target.value)} className={`${inputCls} w-20`} inputMode="decimal" />
          </div>
          <button
            className={btnPrimary}
            onClick={() => {
              const q = Number(newUnder) || 1.32;
              const mother = Array.from({ length: 30 }, (_, i) => ({ odds: q, market: 'UNDER' as const, label: `M${i + 1}` }));
              void run(
                async () => {
                  const c = await createCycle({ name: newName || `Ciclo ${cycles.length + 1}`, s0: Number(newS0) || 2, targetBase: tBank, motherEvents: mother });
                  setSelectedId(c.id);
                  setNewName('');
                },
          ...
          >
            <span className="flex items-center gap-1"><Plus className="w-3.5 h-3.5" /> Nuovo ciclo</span>
          </button>
        </div>
      </section>
      ...
      {selected && (
        <>
          ...
          <section className={cardCls}>
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              <h2 className="text-sm font-semibold text-white">
                {selected.name} · speso {spent}€ · T {target}€ · madre {selected.motherEvents.length}/30
              </h2>
              <div className="flex gap-1.5">
                <button
                  className={btnGhost}
                  onClick={() => {
                    const pool = selected.motherEvents.length > 0
                ...
                <button className={btnGhost} onClick={() => void run(async () => closeCycle(selected.id), 'Ciclo chiuso')}>
                  <span className="flex items-center gap-1"><Check className="w-3 h-3" /> Chiudi</span>
                </button>
                <button className={btnGhost} onClick={() => void run(async () => { await deleteCycle(selected.id); setSelectedId(null); }, 'Ciclo eliminato')}>
                  <span className="flex items-center gap-1 text-red-400"><Trash2 className="w-3 h-3" /> Elimina</span>
                </button>
              </div>
            </div>
            ...
            <div className="flex items-end gap-2 mb-3">
              <div>
                ...
                <input value={topUp} onChange={(e) => setTopUp(e.target.value)} className={`${inputCls} w-28`} inputMode="decimal" placeholder="es. 500" />
              </div>
              <button
                className={btnGhost}
                onClick={() => void run(async () => { await topUpCycle(selected.id, Number(topUp)); setTopUp(''); }, 'Top-up registrato')}
              >
                <span className="flex items-center gap-1"><Wallet className="w-3 h-3" /> Top-up</span>
              </button>
              <span className="text-[10px] font-mono text-[#64748B]">Bankroll rif. ciclo: {selected.bankrollStart}€</span>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[11px] font-mono">
                ...
                <tbody>
                  {selTickets.length === 0 && (
                    <tr><td colSpan={8} className="py-2 text-[#64748B]">Nessun ticket. Piazza la madre o una copertura.</td></tr>
                  )}
                  {selTickets.map((t) => (
                    <tr key={t.id} className="border-t border-[#2D3139] text-white">
                      ...
                      <td className="py-1.5 pr-2">
                        <span className={t.status === 'pending' ? 'text-amber-400' : t.status === 'won' ? 'text-emerald-400' : t.status === 'lost' ? 'text-red-400' : 'text-[#64748B]'}>
                          {t.status}
                        </span>
                      </td>
                      <td className="py-1.5 pr-2">
                        {t.status === 'pending' ? (
                          <div className="flex flex-wrap gap-1">
                            <button className={btnGhost} onClick={() => void run(async () => settleTicket(t.id, 'won'), 'Ticket vinto')}>W</button>
                            <button className={btnGhost} onClick={() => void run(async () => settleTicket(t.id, 'lost'), 'Ticket perso')}>L</button>
                            <button className={btnGhost} onClick={() => void run(async () => settleTicket(t.id, 'void'), 'Ticket void (rimborso)')}>V</button>
                          </div>
                        ) : (
            ...
            {selTickets.filter((t) => t.status === 'pending').map((t) => (
              <div key={t.id} className="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] font-mono">
                ...
                {t.legs.map((l, i) => (
                  <button
                    ...
                    onClick={() => void run(async () => voidTicketLeg(t.id, i, book), `Gamba ${i + 1} voidata (N=${t.legs.length - 1})`)}
                  >
          ...
          {/* Nuovo ticket */}
          <section className={cardCls}>
            <h2 className="text-sm font-semibold text-white mb-2">Piazza ticket (1 attivo per ciclo)</h2>
            <div className="flex flex-wrap items-end gap-2 mb-2">
              <div>
                <label className={labelCls}>Tipo</label>
                <select value={kind} onChange={(e) => setKind(e.target.value as CycleTicketKind)} className={`${inputCls} w-36`}>
              ...
              <div>
                ...
                <select value={bookId || books[0]?.id || ''} onChange={(e) => setBookId(e.target.value)} className={`${inputCls} w-36`}>
                  {books.map((b) => (
              ...
              <div>
                ...
                <input value={bleed} onChange={(e) => setBleed(e.target.value)} className={`${inputCls} w-24`} inputMode="decimal" />
              </div>
            ...
            <div className="space-y-1.5 mb-2">
              {legRows.map((r, i) => (
                <div key={i} className="flex gap-1.5">
                  <input value={r.odds} onChange={(e) => setLegRows((rows) => rows.map((x, j) => (j === i ? { ...x, odds: e.target.value } : x)))} className={`${inputCls} w-24`} inputMode="decimal" placeholder="quota" />
                  <select value={r.market} onChange={(e) => setLegRows((rows) => rows.map((x, j) => (j === i ? { ...x, market: e.target.value as 'OVER' | 'UNDER' } : x)))} className={`${inputCls} w-28`}>
                    <option value="UNDER">UNDER</option>
                    <option value="OVER">OVER</option>
                  </select>
                  <input value={r.label} onChange={(e) => setLegRows((rows) => rows.map((x, j) => (j === i ? { ...x, label: e.target.value } : x)))} className={inputCls} placeholder="label match (guard)" />
                  <button className={btnGhost} onClick={() => setLegRows((rows) => rows.filter((_, j) => j !== i))} disabled={legRows.length <= 1}>✕</button>
                </div>
              ))}
              <button className={btnGhost} onClick={() => setLegRows((rows) => (rows.length >= 30 ? rows : [...rows, emptyLeg()]))}>
                <span className="flex items-center gap-1"><Plus className="w-3 h-3" /> Gamba</span>
              </button>
            </div>
            {parsed.error && <div className="text-[11px] font-mono text-red-400 mb-2">{parsed.error}</div>}
            {preview && preview.feasible && (
              <div className="text-[11px] font-mono text-white mb-2">
                Preview: N={preview.n} finale {preview.finale.toFixed(2)} · stake neutra {preview.stakeNeutral.toFixed(2)}€
                {' → '}piazza <span className="text-[#3B82F6]">{preview.stake.toFixed(2)}€</span>
            ...
            <button
              ...
              onClick={() => {
                if (!parsed.legs || !preview || !book) return;
                void run(
                  async () => {
                    await addTicket(selected.id, { kind, legs: parsed.legs as TicketLeg[], stake: preview.stake, finale: preview.finale, target, book });
                    setLegRows([emptyLeg()]);
                  },
          ...
          <section className={cardCls}>
            ...
            <div className="flex flex-wrap items-end gap-2 mb-2">
              <div>
                ...
                <input value={termN} onChange={(e) => setTermN(e.target.value)} className={`${inputCls} w-24`} inputMode="numeric" />
              </div>
              <div>
                <label className={labelCls}>Over prossimo evento</label>
                <input value={termOver} onChange={(e) => setTermOver(e.target.value)} className={`${inputCls} w-24`} inputMode="decimal" />
              </div>
              <div className="text-[11px] font-mono text-[#64748B]">Tetto lock: speso ≤ {termination?.ceiling != null ? `${termination.ceiling.toFixed(2)}€` : '—'} (s_cap 150€ @2.75)</div>
            </div>
            {termination?.error && <div className="text-[11px] font-mono text-red-400">{termination.error}</div>}
