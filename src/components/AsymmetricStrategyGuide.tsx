...
import { ModelParameters, calculateSteps, roundToFiftyCents, getStepTargetProfit } from '../utils/mathEngine';
...
export const AsymmetricStrategyGuide: React.FC<AsymmetricStrategyGuideProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'how_to_build' | 'asymmetric_advantage' | 'interactive_matrix'>('how_to_build');
  ...
  const comparisonData = useMemo(() => {
    ...
    const capitalPreservSteps = calculateSteps({ ...params, asymmetricMode: 'capital_preservation' });
    ...
    const preservTotalCost = capitalPreservSteps.reduce((acc, s) => acc + s.stake, params.baseStake);
      ...
      <div className="bg-[#141824] border border-[#3B82F6]/30 p-4 rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#E0E2E7]">
            {currentMode === 'flat' && 'Modello Simmetrico: Cerca lo stesso utile (+45€) ovunque, provocando picchi di puntata su step finali.'}
            {currentMode === 'front_loaded' && 'Sbilanciamento Iniziale: Utile maggiorato all\'inizio (+80€) e minimo alla fine (+5€) per abbattere lo stake della singola finale.'}
            {currentMode === 'capital_preservation' && 'Preservazione Capitale: Utile moderato all\'inizio e rigoroso Break-Even (0.00€) sugli ultimi 2 match per eliminare il rischio di capitale.'}
          </p>
        </div>
        ...
        <div className="flex bg-[#0F1117] border border-[#2D3139] rounded-xs p-1 gap-1 shrink-0 font-mono text-xs">
          ...
          <button
            onClick={() => onParamsChange((p) => ({ ...p, asymmetricMode: 'capital_preservation' }))}
            className={`px-3 py-1.5 rounded-xs transition-colors flex items-center gap-1 ${
        ...
        <div className="space-y-6">
          <div className="bg-[#0F1117] border border-[#2D3139] p-5 rounded-sm">
            ...
            <p className="text-xs text-[#94A3B8] leading-relaxed mb-4">
              La strategia <strong>NON richiede di piazzare tutte le multiple prima delle partite</strong>. Si gioca con la logica <strong>"Live Relay a Scalare"</strong> distanziata di 2 ore: il capitale viene immesso solo quando serve.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 font-mono text-xs">
              ...
              <div className="bg-[#1A1D26] border border-[#2D3139] p-4 rounded-xs">
                ...
                <p className="text-[11px] text-[#94A3B8] font-sans leading-relaxed">
                  Piazza la schedina principale con quota moltiplicata (~12x con bonus). Stake iniziale (es. <strong>€{params.baseStake.toFixed(2)}</strong>).
                </p>
              </div>
              ...
              <div className="bg-[#1A1D26] border border-[#2D3139] p-4 rounded-xs">
                ...
                <p className="text-[11px] text-[#94A3B8] font-sans leading-relaxed">
                  Piazza la schedina di copertura <strong>solo per il match in partenza</strong>. Lo stake è calcolato per coprire i costi passati + utile prefissato.
                </p>
              </div>
              ...
              <div className="bg-[#1A1D26] border border-[#2D3139] p-4 rounded-xs">
                ...
                <p className="text-[11px] text-[#94A3B8] font-sans leading-relaxed">
                  Non ci sono più eventi rimanenti. Si piazza una singola Over 3.5 (o con booster) che chiude matematicamente tutte le linee.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-[#0F1117] border border-emerald-500/30 rounded-xs">
                ...
                <ul className="space-y-1.5 text-[11px] text-[#94A3B8] font-sans list-disc list-inside leading-relaxed">
                  <li>La copertura del match corrente è persa (costo minimo, es. 2.50€ o 3.50€).</li>
                  <li><strong>La Schedina Madre è ancora VIVA al 100%!</strong></li>
                  <li>Non devi recuperare nulla subito: attendi l&apos;orario della partita successiva e prepari la Copertura successiva.</li>
                </ul>
              </div>
              ...
              <div className="p-4 bg-[#0F1117] border border-amber-500/30 rounded-xs">
                ...
                <ul className="space-y-1.5 text-[11px] text-[#94A3B8] font-sans list-disc list-inside leading-relaxed">
                  <li>La Schedina Madre muore, <strong>MA la Copertura corrente è VINTA!</strong></li>
                  <li>Poiché la copertura conteneva l&apos;Over appena uscito e tutti i restanti Under, <strong>diventa la NUOVA SCHEDINA MADRE</strong>.</li>
                  <li>Non hai perso il sistema: continui a proteggere questa nuova madre partita per partita.</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#0F1117] border border-[#2D3139] p-5 rounded-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-[#64748B] font-mono">Ottimizzazione del Capitale &amp; Margine</span>
            </div>

            <h3 className="text-base sm:text-lg font-bold text-white mb-2">
              Perché il Dutching Simmetrico (Flat 45€) è un Errore e Come Creare &quot;Spazio di Cassa&quot;
            </h3>
            <p className="text-xs sm:text-sm text-[#94A3B8] leading-relaxed max-w-4xl mb-6">
              Nel dutching tradizionale, chi gioca imposta lo <strong>stesso utile identico</strong> su tutte le schedine (es. +45€).
              Ma all&apos;inizio le quote sono <strong>altissime (27x - 20x)</strong> e alla fine sono <strong>bassissime (2.75x)</strong>.
              Pretendere 45€ sulla singola finale ti costringe a puntare oltre <strong>111,50€</strong>, prosciugando la cassa.
            </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 font-mono text-xs">
              ...
              <div className={`p-4 rounded-xs border transition-all ${
                currentMode === 'flat' ? 'bg-[#1A1D26] border-[#3B82F6]' : 'bg-[#0F1117] border-[#2D3139]'
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] text-[#64748B] uppercase">Modello 1</span>
                  <span className="text-[10px] px-1.5 py-0.2 bg-zinc-800 text-zinc-300 rounded-xs">Simmetrico</span>
                </div>
                <h4 className="text-white font-bold mb-1">Target Fisso Costante</h4>
                <p className="text-[11px] text-[#94A3B8] font-sans mb-3">
                  Utile netto costante di <strong>+€{params.targetProfit.toFixed(2)}</strong> su tutti gli step, dal primo all&apos;ultimo.
                </p>
                <div className="space-y-1.5 border-t border-[#2D3139] pt-3 text-[11px]">
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Step 1:</span>
                    <span className="text-white font-bold">€{comparisonData.flatSteps[0]?.stake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Singola Finale:</span>
                    <span className="text-red-400 font-bold">€{comparisonData.flatFinalStake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Costo Totale Potenziale:</span>
                    <span className="text-orange-400 font-bold">€{comparisonData.flatTotalCost.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-4 rounded-xs border transition-all ${
                currentMode === 'front_loaded' ? 'bg-[#1A1D26] border-amber-500 shadow-sm shadow-amber-500/10' : 'bg-[#0F1117] border-[#2D3139]'
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] text-amber-400 uppercase font-bold">Modello 2 (Consigliato)</span>
                  <span className="text-[10px] px-1.5 py-0.2 bg-amber-950/50 text-amber-300 border border-amber-500/40 rounded-xs">Sbilanciato</span>
                </div>
                <h4 className="text-white font-bold mb-1">Sbilanciamento Iniziale</h4>
                <p className="text-[11px] text-[#94A3B8] font-sans mb-3">
                  Punti a <strong>+€80€</strong> all&apos;inizio (quando la quota è 27x e costa solo 1.50€ in più!), e scendi a <strong>+€5€</strong> alla fine.
                </p>
                <div className="space-y-1.5 border-t border-[#2D3139] pt-3 text-[11px]">
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Step 1:</span>
                    <span className="text-emerald-400 font-bold">€{comparisonData.frontLoadedSteps[0]?.stake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Singola Finale:</span>
                    <span className="text-emerald-400 font-bold">€{comparisonData.frontFinalStake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between border-t border-[#2D3139] pt-1">
                    ...
                    <span className="text-amber-300 font-bold">-€{comparisonData.stakeSavedOnFinal.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-4 rounded-xs border transition-all ${
                currentMode === 'capital_preservation' ? 'bg-[#1A1D26] border-emerald-500' : 'bg-[#0F1117] border-[#2D3139]'
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] text-emerald-400 uppercase">Modello 3</span>
                  <span className="text-[10px] px-1.5 py-0.2 bg-emerald-950/50 text-emerald-300 border border-emerald-500/40 rounded-xs">Salva Capitale</span>
                </div>
                <h4 className="text-white font-bold mb-1">Preservazione di Cassa</h4>
                <p className="text-[11px] text-[#94A3B8] font-sans mb-3">
                  Utile moderato all&apos;inizio e <strong>Break-Even puro (€0.00 di utile)</strong> sulla finale: recupera solo le spese pregresse.
                </p>
                <div className="space-y-1.5 border-t border-[#2D3139] pt-3 text-[11px]">
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Step 1:</span>
                    <span className="text-white font-bold">€{comparisonData.capitalPreservSteps[0]?.stake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[#64748B]">Puntata Singola Finale:</span>
                    <span className="text-emerald-400 font-bold">€{comparisonData.preservFinalStake.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between border-t border-[#2D3139] pt-1">
                    <span className="text-emerald-400 font-bold">Risparmio Massimale:</span>
                    <span className="text-emerald-300 font-bold">-€{comparisonData.stakeSavedPreservation.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-[#1A1D26] border border-[#2D3139] rounded-xs font-mono text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-[11px] text-[#94A3B8] font-sans">
                <div className="p-2.5 bg-[#0F1117] border border-[#2D3139] rounded-xs">
                  ...
                  Metti nei primi 2-3 slot orari le partite con la quota Under più bassa (es. 1.25 o 1.28). Se passano, hai azzerato il rischio senza spendere nulla.
                </div>
                <div className="p-2.5 bg-[#0F1117] border border-[#2D3139] rounded-xs">
                  <strong className="text-white block mb-1">2. Accetta il Break-Even Finale</strong>
                  All&apos;ultimo match non cercare il colpo grosso: l&apos;obiettivo unico deve essere <strong>recuperare il capitale speso</strong>.
                </div>
                <div className="p-2.5 bg-[#0F1117] border border-[#2D3139] rounded-xs">
                  <strong className="text-white block mb-1">3. Cuscino Booster 1.10</strong>
                  Aggiungi un evento a 1.10 sulla singola finale: riduce la puntata di altri 15€-20€ e ripristina il bonus multipla se sei a 4 eventi.
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-3 border-b border-[#2D3139]">
            <div>
              ...
              <p className="text-xs text-[#94A3B8] mt-0.5">
                Tutti i calcoli sono matematicamente arrotondati all&apos;incremento superiore di 0,50€
              </p>
            </div>

            <div className="text-xs font-mono text-[#64748B]">
              Base Iniziale: <strong className="text-white">€{params.baseStake.toFixed(2)}</strong> &bull; Target: <strong className="text-white">€{params.targetProfit.toFixed(2)}</strong>
            </div>
          </div>
            ...
            <table className="w-full text-left font-mono text-xs border-collapse">
              ...
              <tbody className="divide-y divide-[#20242C]">
                {comparisonData.flatSteps.map((step, idx) => {
                  ...
                  const frontTarget = getStepTargetProfit(step.step, params.totalEvents, params.targetProfit, 'front_loaded');
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2.5 text-right">
                        {diff > 0 ? (
                          <span className="text-emerald-400 font-bold">+€{diff.toFixed(2)} risparmiati</span>
                        ) : diff < 0 ? (
                          <span className="text-[#64748B]">-€{Math.abs(diff).toFixed(2)} (anticipo)</span>
                        ) : (
